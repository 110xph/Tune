...
from sql_explain import sql_explainer
...
# regression
def baseline_model(num_feature=38, num_output=17):
    # create model
    model = Sequential()
    model.add(Dense(num_feature * 2, input_dim=num_feature, kernel_initializer='normal', activation='relu'))
    model.add(Dense(num_feature * 2, kernel_initializer='normal', activation='relu'))
...
class SqlParser:
    ...
    def __init__(self, db, benchmark='sysbench', sql = '10a.sql', cur_op='oltp_read_write.lua', num_event=1000, p_r_range=0.6, p_u_index=0.2, p_i=0.1, p_d=0.1):
        if benchmark == 'sysbench':
            ### Sysbench ==s ingle table operations
            ## elements: shell_cmd; sql_vector
            ...
            ### Sysbench ==s ingle table operationsz
            ## elements: shell_cmd; sql_vector
            self.SQL_NAMES = [sql]
            ...
            self.explainer = sql_explainer(sql, db=db)

            self.sql_vector = np.array(self.explainer.explain())
            ...
            ######### Prepare training data
            fs = open(q_path + "sql_vector.txt", 'r')
            df = pandas.read_csv(fs, sep='\t', header=None)
            lt_sql = df.values
            # seperate into input X and output Y
            sql_X = lt_sql[:, 0:38]  # op_type   events  table_size
            sql_Y = lt_sql[:, 38:]
            print("### read_sql:")
            print(sql_X[0])
            print(sql_Y[0])
            ...
            # estimators.append(('standardize', StandardScaler()))
            # estimators.append(('mlp', KerasRegressor(build_fn=baseline_model, epochs=50, batch_size=50, verbose=0)))
            self.estimator = KerasRegressor(build_fn=baseline_model, epochs=10, batch_size=50, verbose=1)  # epochs
            kfold = KFold(n_splits=10, random_state=seed)       # n_splites packs, with 1 packet as the test set
            results = cross_val_score(self.estimator, X_train, Y_train, cv=kfold)
    ...
    def predict_sql_resource(self):
        cur_state = np.array([1]*38)
        cur_state = cur_state.reshape((1, cur_state.shape[0]))
        return self.estimator.predict(cur_state)  # input : np.array([[...]])      (sq_type, num_events, C, aggregation, in-mem)

        # Predict sql convert
        # inner_metric_change   np.array
        self.sql_vector = np.array(self.explainer.explain())
        self.sql_vector = np.array([1]*38)
        print(self.sql_vector)
