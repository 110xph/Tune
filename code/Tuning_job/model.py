...
class ActorCritic:
    ...
    def create_actor_model(self):
        ...
        output = Dense(self.env.action_space.shape[0], activation=self.target_range)(d1)
    ...
    def create_critic_model(self):
        # input: action + (state + change_state)      30  14  14
        # (dense dense)->dense->dense->BN->dense
        state_input = Input(shape=self.env.observation_space.shape)
        state_h1 = Dense(58)(state_input)
        # state_h2 = Dense(13)(state_h1)

        action_input = Input(shape=self.env.action_space.shape)
        action_h1 = Dense(58)(action_input)  #

        merged = Add()([state_h1, action_h1])
        merged_h1 = Dense(116, activation='relu')(merged)
        h2 = Dense(58)(merged_h1)
        h3 = Dense(29, activation='tanh')(h2)
        d1 = Dropout(0.3)(h3)
    ...
    def target_range(self, x):

        x02 = K.tanh(x) + 1  # x in range(0,2)
        scale = (self.env.a_high - self.env.a_low) / 2.
        return x02 * scale + self.env.a_low
