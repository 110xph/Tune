...
sql_vector = [0]*v_len
repeat_time = 1
q_path = '/home/zxh/join-order-benchmark-master/'  # the path to sql statements
v_path = '/home/zxh/join-order-benchmark-master/'  # the path to generated feature vectors
...
for i in range(33):
    for e in ['a', 'b', 'c', 'd', 'e', 'f']:
        ...
        if os.path.isfile(f_name):
            ...
            sql2v(sql)
            # run on database
            l1 = db.fetch_internal_metrics()
            for j in range(repeat_time):
                with db.connection.cursor() as cursor:
                    cursor.execute(sql)                                                         ### Blocking
                    print("### %d %s ###"%(i, e))
                    print(cursor.fetchall())
            l2 = db.fetch_internal_metrics()

            for k, t in enumerate(l2):
                vfs.write("%d\t"%abs(t-l1[k]))                                                  ### change during execution period
            vfs.write("\n")
